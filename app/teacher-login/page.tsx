          ...
          {showForgot && (
            <form onSubmit={handleForgotPassword} className="mt-4 space-y-3 border-t border-border pt-4">
              ...
              <input
                ...
                placeholder="teacher@college.edu"
                className="w-full px-4 py-2 rounded-lg border border-border bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              />
              {resetMessage && (
                <p className={`text-sm ${resetStatus === "success" ? "text-green-500" : "text-red-500"}`}>{resetMessage}</p>
              )}
              <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 py-2 h-auto text-sm">
                Send Reset Link
              </Button>
            </form>
          )}
