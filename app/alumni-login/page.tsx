...
export default function AlumniLoginPage() {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-background flex flex-col items-center justify-center px-4">
            ...
            <div className="absolute bottom-0 left-0 -mb-20 -ml-20 w-80 h-80 bg-accent/20 rounded-full blur-[100px] opacity-60 pointer-events-none animate-float delay-200"></div>
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                {/* Login Card */}
                <div className="glass-card rounded-2xl p-8 premium-shadow opacity-0-init animate-scale-in delay-200 border-t-4 border-t-primary">
                    ...
                    {showForgot && (
                        <form onSubmit={handleForgotPassword} className="mt-4 space-y-3 border-t border-border pt-4">
                            ...
                            <input
                                ...
                                placeholder="alumni@company.com"
                                className="w-full px-4 py-2 rounded-lg border border-border bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-purple-600 text-sm"
                            />
                            {resetMessage && (
                                <p className={`text-sm ${resetStatus === "success" ? "text-green-500" : "text-red-500"}`}>{resetMessage}</p>
                            )}
                            <Button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 py-2 h-auto text-sm">
                                Send Reset Link
                            </Button>
                        </form>
                    )}
